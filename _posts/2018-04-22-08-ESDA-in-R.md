...
One way to conceptualize space for ESDA is by distance.  The lagged scatter plot bins pairs of observations into distance classes. We will use data collected from EPA's Wadeable Streams Assessment.  The points, or sites, are from streams sampled in three ecoregions:  Northern Plains, Temperate Plains, and Southern Plains.  The expectation is the correlation between sites will be highest in the smallest distance class, and that with increasing distance classes the correlations will be weaker.  The Wadeable Streams Assessment is a spatially-balanced, probabilistic survey designed and analyzed using the R package spsurvey.
...
#using rgdal package so can read ESRI shapelfile with its projection.  The shapefile needs to
